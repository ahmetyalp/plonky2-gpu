...
fn fill_subtree2<F: RichField, H: Hasher<F>>(
    idx: usize,
    digests_buf: &mut [MaybeUninit<H::Hash>],
    leaves: &[Vec<F>],
) -> H::Hash {
    ...
    if digests_buf.is_empty() {
        ...
        let (left_digest, right_digest) = maybe_rayon::join(
            || fill_subtree3::<F, H>(idx, 0, left_digests_buf, left_leaves),
            || fill_subtree3::<F, H>(idx, leaves.len()/2, right_digests_buf, right_leaves),
        );
        ...
        // let hex_string: String = unsafe{std::mem::transmute::<&_, &[u8;32]>(&left_digest)}.iter().map(|byte| format!("{:02x}", byte)).collect();
        // let resultL: String = hex_string.chars()
        //     .collect::<Vec<char>>()
        //     .chunks(16)
        //     .map(|chunk| chunk.iter().collect::<String>())
        //     .collect::<Vec<String>>()
        //     .join(", ");
        // let hex_string: String = unsafe{std::mem::transmute::<&_, &[u8;32]>(&right_digest)}.iter().map(|byte| format!("{:02x}", byte)).collect();
        // let resultR: String = hex_string.chars()
        //     .collect::<Vec<char>>()
        //     .chunks(16)
        //     .map(|chunk| chunk.iter().collect::<String>())
        //     .collect::<Vec<String>>()
        //     .join(", ");
        //
        // println!("cpu hash {}: left:{}, right: {}", idx, resultL, resultR);

        H::two_to_one(left_digest, right_digest)
    }
}

fn fill_subtree3<F: RichField, H: Hasher<F>>(
    idx: usize,
    base_idx: usize,
    digests_buf: &mut [MaybeUninit<H::Hash>],
    leaves: &[Vec<F>],
    ...
    if digests_buf.is_empty() {
        ...
        //     for i in 0..8 {
        //         println!("cpu leaves{}: {}",
        //                  // leaves[0][0].iter().map(|f| format!("{:016X}", unsafe{*std::mem::transmute::<&F, &u64>(f)})).collect().join(",")
        //                  i, leaves[0][i]
        //         );
        //
        //     }
        // }
        let res = H::hash_or_noop(&leaves[0]);
        // if base_idx == 0 && idx == 0
        // {
        //     // println!("hash: {:?}",  unsafe{std::mem::transmute::<&_, &[u8;32]>(&res)})
        //     let hex_string: String = unsafe{std::mem::transmute::<&_, &[u8;32]>(&res)}.iter().map(|byte| format!("{:02x}", byte)).collect();
        ...
        //         .collect::<Vec<String>>()
        //         .join(", ");
        //     println!("capid: {}, idx: {}, hash: {}", idx, base_idx,  result);
        //
        // }
        res
    } else {
        ...
        let (left_digest, right_digest) = maybe_rayon::join(
            || fill_subtree3::<F, H>(idx, base_idx, left_digests_buf, left_leaves),
            || fill_subtree3::<F, H>(idx, base_idx+leaves.len()/2, right_digests_buf, right_leaves),
        );
...
fn fill_digests_buf2<F: RichField, H: Hasher<F>>(
    digests_buf: &mut [MaybeUninit<H::Hash>],
    cap_buf: &mut [MaybeUninit<H::Hash>],
    leaves: &[Vec<F>],
    cap_height: usize,
    ...
    assert_eq!(digests_chunks.len(), cap_buf.len());
    assert_eq!(digests_chunks.len(), leaves_chunks.len());
    digests_chunks.zip(cap_buf).zip(leaves_chunks).enumerate().for_each(
        |(idx, ((subtree_digests, subtree_cap), subtree_leaves))| {
            // We have `1 << cap_height` sub-trees, one for each entry in `cap`. They are totally
            // independent, so we schedule one task for each. `digests_buf` and `leaves` are split
            // into `1 << cap_height` slices, one for each sub-tree.
            subtree_cap.write(fill_subtree2::<F, H>(idx, subtree_digests, subtree_leaves));
        },
    );
}
...
impl<F: RichField, H: Hasher<F>> MerkleTree<F, H> {
    ...
    pub fn new2(leaves: Vec<Vec<F>>, cap_height: usize) -> Self {
        let log2_leaves_len = log2_strict(leaves.len());
        assert!(
            cap_height <= log2_leaves_len,
            "cap_height={} should be at most log2(leaves.len())={}",
        ...
        let digests_buf = capacity_up_to_mut(&mut digests, num_digests);
        let cap_buf = capacity_up_to_mut(&mut cap, len_cap);
        fill_digests_buf2::<F, H>(digests_buf, cap_buf, &leaves[..], cap_height);
        ...
        // for (idx, h) in digests.iter().enumerate() {
        //     // println!("hash: {:?}",  unsafe{std::mem::transmute::<&_, &[u8;32]>(&res)})
        ...
        // for (idx, h) in cap.iter().enumerate() {
        //     // println!("hash: {:?}",  unsafe{std::mem::transmute::<&_, &[u8;32]>(&res)})
        //     let hex_string: String = unsafe{std::mem::transmute::<&_, &[u8;32]>(h)}.iter().map(|byte| format!("{:02x}", byte)).collect();
        //     let result: String = hex_string.chars()
        //         .collect::<Vec<char>>()
